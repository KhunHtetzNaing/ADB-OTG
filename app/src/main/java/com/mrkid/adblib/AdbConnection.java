...
public class AdbConnection implements Closeable {
    ...
    public static AdbConnection create(Socket socket, AdbCrypto crypto) throws IOException {
        ...
        newConn.channel = new TcpChannel(socket);
     ...
     * crypto object specified.
     *
     * @param connection The socket that the connection will use for communcation.
     * @param intf
     * @param crypto     The crypto object that stores the key pair for authentication.
     * @return A new AdbConnection object.
     * @throws IOException If there is a socket error
     */
    public static AdbConnection create(UsbDeviceConnection connection, UsbInterface intf, AdbCrypto crypto) throws IOException {
        AdbConnection newConn = new AdbConnection();

        newConn.crypto = crypto;

        newConn.channel = new UsbChannel(connection, intf);
            ...
            public void run() {
                while (!connectionThread.isInterrupted()) {
                    try {
                        ...
                        AdbProtocol.AdbMessage msg = AdbProtocol.AdbMessage.parseAdbMessage(channel);

                        Log.d(Const.TAG, "msg :" + msg);

						/* Verify magic and checksum */
                        if (!AdbProtocol.validateMessage(msg)) {
                            Log.w(Const.TAG, "msg invalid:" + msg);
                            continue;
                        }
                        ...
                        /* Stream-oriented commands */
							...
							/* Get the stream object corresponding to the message */
                            ...
                            case AdbProtocol.CMD_AUTH:
                                ...
                                AdbProtocol.AdbMessage message;
								...
								/* This is an authentication challenge */
									...
									/* We've already tried our signature, so send our public key */
                                        message = AdbProtocol.generateAuth(AdbProtocol.AUTH_TYPE_RSA_PUBLIC,
                                                conn.crypto.getAdbPublicKeyPayload());
                                    } else {
									/* We'll sign the token */
                                        message = AdbProtocol.generateAuth(AdbProtocol.AUTH_TYPE_SIGNATURE,
                                                conn.crypto.signAdbTokenPayload(msg.getPayload()));
                                        conn.sentSignature = true;
                                    }

								/* Write the AUTH reply */
                                    conn.channel.write(message);
                                }
                                break;
                            ...
                            case AdbProtocol.CMD_CNXN:
								...
								/* Mark us as connected and unwait anyone waiting on the connection */
                                    conn.connected = true;
                                    ((Object) conn).notifyAll();
                                }
                                break;

                            default:
							/* Unrecognized message, just drop it */
                                break;
                        }
                    } catch (Exception e) {
    ...
    public AdbStream open(String destination) throws UnsupportedEncodingException, IOException, InterruptedException {
		...
		/* Send the open */
        boolean write = channel.write(AdbProtocol.generateOpen(localId, destination));
        if (write) {
            Log.d(Const.TAG, "queue open " + destination);
        } else {
            throw new ConnectException("fail to write open request");
        }
